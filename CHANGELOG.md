...
- 一键安装（免费层）：`add <github 地址>` 从 GitHub 装 skill，`installed` 看装过什么，
  ...
  - **同名绝不覆盖**：目标目录已存在即硬阻塞，没有 `--force` 覆盖开关，只能 `--as` 改名或先 `remove`
  - **装前先出计划**：默认只打印来源仓库 / 提交号 / 目标路径 / 冲突，写盘必须显式 `--yes`
  - **来源存证**：`~/.skill-picker/installed.json` 记源 URL、ref、commit、逐文件 sha256
  - **装完体检**：落盘逐文件比哈希（不一致自动回滚）→ 重扫 → 确认进了 catalog → 五道门禁
  - **回滚只碰自己的东西**：改动过的文件默认拒删（需 `--force`），装完之后用户新增的文件一律保留
- 解包安全门（这条链路有用户开发机的写权限，按不可信输入处理）：拒绝跳目录、绝对路径、
  反斜杠、控制字符、符号链接、仅大小写不同的撞名、Windows 保留名与尾随点、超深路径，
  以及下载/解包体积与文件数上限；声明体积与实际解出大小不符也拒——否则体积门禁形同虚设
- monorepo 必须显式选目录：feed 里 93.6% 是 monorepo 子条目，包内多个 `SKILL.md` 时
  列出候选让人挑，不替用户猜
- `install_roots()`：安装目标只认末级为 `skills` 的目录，排除插件缓存与宿主内置目录
  （往那些地方写会被宿主自己的更新盖掉）；一个宿主标签对应多个根时取 skill 更多的那个。
  默认宿主 = skill 最多的那个，并列按名字定序
...
### Changed
- 只读铁律的措辞改准：`add` 会往宿主目录写新文件，`remove` 会删文件，
  原来那句「不修改任何已有 skill 文件」照字面读会被当成骗人。现在写成
  「只新建、绝不覆盖；只删自己写过且没被改动过的」，并在 README / AGENTS.md 同步
- README 的「纯本地」口径同步：找技能 / 理技能仍零出网，但 `add` 要连 GitHub，
